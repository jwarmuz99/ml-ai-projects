...
def leaf_node(board):
    ...
    return winning_combination(board, BLUE_AI) or winning_combination(board, RED_AI) or len(all_valid_columns(board)) == 0
...
def minimax(board, depth, alpha, beta, maximizer, player_color):
    ...
    if depth == 0 or is_terminal:
        ...
        if is_terminal:
            ...
            if winning_combination(board, player_color):
                return (None, float("inf"))
            # assigning the utility value in case user won
            elif winning_combination(board, -player_color):
                return (None, float("-inf"))
        ...
        # if we reached the depth limit, we want to calculate the utility of that state
            ...
            return (None, calculate_utility(board, player_color))
    ...
    if maximizer:
        ...
        for col in valid_locations:
            ...
            tmp_board = drop_disc(tmp_board, col, player_color)
            # and go to the next depth level by recursively calling the minimax function
            new_utility = minimax(tmp_board, depth-1, alpha, beta, False, player_color)[1]
    ...
    # run the opposite for the MIN player
        ...
        for col in valid_locations:
            tmp_board = board.copy()
            tmp_board = drop_disc(tmp_board, col, player_color)
            new_utility = minimax(tmp_board, depth-1, alpha, beta, True, player_color)[1]
            if new_utility < value:
                value = new_utility
                column = col
            beta = min(beta, value)
